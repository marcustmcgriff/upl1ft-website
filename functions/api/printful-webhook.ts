...
function buildShippedEmailHtml(data: {
  ...
  const shipDateHtml = data.shipDate
    ? `<tr><td style="padding: 8px 0; color: #999;">Shipped</td><td style="padding: 8px 0; color: #fff; text-align: right;">${formatDate(data.shipDate)}</td></tr>`
    : "";

  const estDeliveryHtml = data.estimatedDelivery
    ? `<tr><td style="padding: 8px 0; color: #999;">Estimated Delivery</td><td style="padding: 8px 0; color: #C9A227; text-align: right; font-weight: bold;">${formatDate(data.estimatedDelivery)}</td></tr>`
    : "";
...
<body style="margin: 0; padding: 0; background: #0a0a0a; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
  <div style="max-width: 600px; margin: 0 auto; padding: 40px 20px;">
    ...
    <div style="background: #111; padding: 24px; margin-bottom: 24px;">
      ...
      <table style="width: 100%;">
        <tr>
          ...
          <td style="padding: 8px 0; color: #fff; text-align: right;">${escapeHtml(data.carrier)}</td>
        </tr>
        <tr>
          <td style="padding: 8px 0; color: #999;">Tracking Number</td>
          <td style="padding: 8px 0; color: #C9A227; text-align: right; font-family: monospace;">${escapeHtml(data.trackingNumber)}</td>
        </tr>
        ${shipDateHtml}
        ${estDeliveryHtml}
      </table>
    ...
    <div style="text-align: center; margin-bottom: 24px;">
      <a href="${data.siteUrl}/orders/track?token=${data.trackingToken}" style="display: inline-block; background: #C9A227; color: #000; padding: 16px 40px; text-decoration: none; font-weight: bold; text-transform: uppercase; letter-spacing: 2px; font-size: 14px;">Track Your Order</a>
    </div>

    <!-- Continue Shopping -->
    <div style="text-align: center; margin-bottom: 40px;">
      <a href="${data.siteUrl}/shop" style="color: #C9A227; text-decoration: none; font-size: 14px; letter-spacing: 1px;">Continue Shopping &rarr;</a>
    </div>
    ...
    <div style="text-align: center; border-top: 1px solid #222; padding-top: 24px;">
      <p style="color: #666; font-size: 12px; margin: 0;">
        ...
        <a href="${data.siteUrl}" style="color: #C9A227; text-decoration: none;">upl1ft.org</a>
      </p>
...
<body style="margin: 0; padding: 0; background: #0a0a0a; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
  <div style="max-width: 600px; margin: 0 auto; padding: 40px 20px;">
    ...
    <div style="text-align: center; margin-bottom: 32px;">
      <div style="font-size: 48px; margin-bottom: 12px;">&#10003;</div>
      <h2 style="color: #C9A227; font-size: 22px; letter-spacing: 2px; text-transform: uppercase; margin: 0;">Your Order Has Arrived</h2>
      <p style="color: #999; margin-top: 8px;">Wear it with purpose, ${escapeHtml(data.shippingName.split(" ")[0])}.</p>
    </div>

    <!-- Items Delivered -->
    <div style="background: #111; padding: 24px; margin-bottom: 24px;">
      <h3 style="color: #C9A227; font-size: 13px; text-transform: uppercase; letter-spacing: 2px; margin: 0 0 16px 0;">What Was Delivered</h3>
      <table style="width: 100%; border-collapse: collapse;">
        ${itemsHtml}
      </table>
    </div>
    ...
    <div style="text-align: center; margin-bottom: 16px;">
      <a href="${data.siteUrl}/shop" style="display: inline-block; background: #C9A227; color: #000; padding: 16px 40px; text-decoration: none; font-weight: bold; text-transform: uppercase; letter-spacing: 2px; font-size: 14px;">Explore New Drops</a>
    </div>

    <div style="text-align: center; margin-bottom: 40px;">
      <a href="${data.siteUrl}/orders/track?token=${data.trackingToken}" style="color: #C9A227; text-decoration: none; font-size: 14px; letter-spacing: 1px;">View Order Details &rarr;</a>
    </div>
